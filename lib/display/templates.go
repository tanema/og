...
{{- end}}`
	...
	dotsSeparateTemplate = `{{range $name, $pkg := .Packages }}{{- if gt (len $pkg.Results) 0 -}}
{{ $name | bold }} {{if $pkg.Cached}}(cached){{end}} {{range $_name, $test := $pkg.Results -}}
	{{- if eq $test.State "run" "cont" -}}
	{{"●" | faint | cyan}}
	{{- else if eq $test.State "pass" -}}
	{{"●" | green}}
...
{{- end -}}`
	...
	namesPackageTemplate = `{{range $name, $pkg := .Packages }}{{- if gt (len $pkg.Results) 0 -}}
...
{{- else if eq $pkg.State "skip" -}}{{"SKIP" | bgBlue | white}}
